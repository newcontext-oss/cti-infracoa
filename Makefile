...
test-coa:
	(ls *.txt *.json stixcoa.py) | (. ./p/bin/activate && ~/src/eradman-entr-c15b0be493fc/entr sh -c 'python -m coverage run -m unittest stixcoa && python -m coverage report -m --omit=p/\*')
